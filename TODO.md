...
Phase 1-4 全部完成，Phase 4.5 全部收口（P0/P1/P2 完成），Phase 5 P0/P1/P2 几乎全部完成（721 tests passed，零回归）。仅剩样本语料多样化（图纸/水印需人工获取）。

GAPS.md 全量审计（2026-04-29）发现新缺陷已并入本文件。

本轮收口：Phase 5 P1 测试套件加固（+14 tests，721 total）+ Phase 5 P0 Reviewer 命中条件复核（零阈值调整）+ Phase 5 P2 代码卫生 5/6 项。
...
```
Phase 1 ✅ → Phase 2 ✅ → Phase 3 ✅ → Phase 4 ✅ → Phase 4.5 ✅
                                               ↓
                                       Phase 5 ✅ (除阻塞/暂缓)
                                               ↓
                                       Phase 6 (选做项，允许永远不做)
...
- [x] **Reviewer 命中条件复核（第二轮，扩样本后）** — 2026-04-30 完成，零阈值调整
  - 全量审计 10 个 `_review_*` 函数 × 17 份样本，无过拟合
  - `_find_sentence_like_tags` 18 字符阈值在 GB 12514.1-2005 上可能偏紧（B 级 7 分），建议后续检查实际标签；其余 9 个函数阈值无需修改
  - `summary_fallback_explained` 在 4/5 Phase 5 新增样本上触发，属于 summarizer 层问题非 reviewer 误判
  - 零代码变更，纯审计确认
...
- [ ] **样本语料多样化** — **部分完成，图纸/水印阻塞**
  - 德文原版：已覆盖（DIN 14313-1989 在基线，16 份 DIN PDF 在 input/）
  - 图纸型 PDF：缺失 — 需获取至少 1 份 blueprints/engineering drawings
  - 水印污染样本：缺失 — 需获取至少 1 份含 URL/广告水印的 PDF
  - *阻塞原因：* 无法自动生成样本，需人工获取
  - *依赖：* Phase 3 slow baseline 鲁棒性已就绪

### P1 — 测试套件加固

- [x] **ScannedBaselineTests 改为真测试** — 2026-04-30 收口（Phase 4.5 P2）
  - `test_scanned_baseline` snapshot 段已修复为真测试（missing_deductions + _UPDATE_SNAPSHOTS gate + 比较断言）
  - *文件：* `tests/test_sample_score_baseline.py:330-358`

- [x] **单方法测试文件扩充** — 2026-04-30 收口
  - `test_export_contract.py`: 1→5 tests（空文档/OCR置信度/process_log不过滤/中文roundtrip）
  - `test_pipeline_ocr_summary.py`: 1→6 tests（无OCR轮次/部分完成/多轮聚合/失败原因/全字段）
  - *文件：* `tests/test_export_contract.py`、`tests/test_pipeline_ocr_summary.py`

- [x] **profiler 语言检测测试加强** — 2026-04-30 收口
  - 现有 2 个测试加强为验证 `_detect_language()` 返回值
  - 新增 5 个测试：中文检测/德文变音/空文本/模糊阈值/德文优先
