...
5. **契约 / 待办 / 历史 / 教训分文件维护。** 修改架构契约改 `First Principles.md`；时间线追加进 `CHANGELOG.md`；待办流转改 `TODO.md`；新教训累积进 `LESSONS.md`。FP 自身只承载契约不变量，不再混入 changelog / TODO。
...
├── requirements.txt
├── First Principles.md     # 架构契约与不变量
├── CHANGELOG.md           # 历史落地时间线（从 FP §11 拆出）
├── TODO.md                # 待办、缺陷与不立项归档
├── LESSONS.md             # 工作流与工程教训索引
├── README.md               # 快速上手说明
├── src/
...
通过（总分 ≥ 85 且无红线触发）？
    ...
    └─ 否 → 识别问题类型，定向修正
               ├─ 红线：正文主链缺失 → 重跑 parser
               ├─ 红线：文本层不足需要OCR → 调 PaddleOCR 对低文本层页识别 → 执行 OCR 页级评估 → 仅将评估合格页注入 PipelineContext.force_ocr_pages → 重跑 parser
...
> 最近整理：2026-04-27（plan07 收口）。本节只保留文档地图；历史、待办、教训已拆到专门文件。

### 文档地图

- **近期落地与稳定能力**：见 [CHANGELOG.md](./CHANGELOG.md)
- **下一阶段待办 / 本轮缺陷 / 不立项归档**：见 [TODO.md](./TODO.md)
- **结构层教训**：见 [LESSONS.md](./LESSONS.md)

### 当前指针

- **最近一轮**：2026042406plan（已收口，commit `8273a5a`）
- **当前在做**：2026042701plan / plan07（FP 四拆 + OCR 前移）

### 维护约定

- 架构契约、输出规格、流程不变量仍写入 `First Principles.md`。
- 时间线追加到 `CHANGELOG.md`；待办状态流转到 `TODO.md`；跨轮复用教训沉淀到 `LESSONS.md`。
- 本节只做入口索引，避免 FP 再次膨胀为 changelog / TODO 混合体。
- **条件句闭环强制记录**：任何含 `if X then Y` 的 plan 执行完毕后，必须在本节末尾追加一行记录 X-check 结果（"X 成立 / 不成立，结论："）。存量条目在收口时追填。
